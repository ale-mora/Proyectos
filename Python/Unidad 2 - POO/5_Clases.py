...
mensaje = "hola mundo"
print(type(mensaje))
# Type es una función que permite ver el tipo de algún dato.
...
# Representa una entidad o cosa con características y acciones asociadas.
# Los objetos pueden ser creados a partir de clases.
# Cada objeto tiene atributos (datos) y métodos (funciones) que definen su comportamiento.

# Atributo:
# Un atributo es una característica o propiedad de un objeto.
# Por ejemplo, si creamos una clase llamada Persona, los atributos podrían ser nombre, edad, género, etc.

# Método:
# Un método es una acción o función que realiza un objeto.
# Por ejemplo, si tenemos una clase Coche, los métodos podrían ser arrancar, detener, acelerar, etc.

# Clase:
# Una clase es una plantilla genérica para crear objetos. (plano de contrucción)
# Define la estructura y el comportamiento que los objetos tendrán.
# Proporciona variables iniciales de estado (atributos) e implementaciones de comportamiento (métodos).

# Instancia:
# Una instancia es una copia específica de una clase con todo su contenido.
# Por ejemplo, si creamos una clase Mascota, una instancia podría ser Mateo = Mascota("Mateo", "Gato").
